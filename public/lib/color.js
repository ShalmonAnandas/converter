...
export function checkContrast(foreground, background) {
  ...
  const tone = (threshold) => (ratio >= threshold ? "good" : "bad");
  return report([
    ["Foreground", toHex(front, { short: false })],
    ["Background", toHex(back, { short: false })],
    ["Ratio", `${round(ratio, 2)}:1`],
    ["Verdict", wcagVerdict(ratio), ratio >= 4.5 ? "good" : "bad"],
    "",
    "WCAG 2.2",
    ["AA normal text", `${check(4.5)} — needs 4.5:1`, tone(4.5)],
    ["AA large text", `${check(3)} — needs 3:1`, tone(3)],
    ["AA UI components", `${check(3)} — needs 3:1`, tone(3)],
    ["AAA normal text", `${check(7)} — needs 7:1`, tone(7)],
    ["AAA large text", `${check(4.5)} — needs 4.5:1`, tone(4.5)],
  ]);
}
...
export function buildPalette(input, scheme = "shades") {
  ...
  const entries = build();
  return report(entries.map(([label, hex]) => {
    const { r, g, b } = parseColor(hex);
    return [label, `${hex}  rgb(${r} ${g} ${b})`];
  }));
}
